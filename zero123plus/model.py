...
class MVDiffusion(pl.LightningModule):
    ...
    def prepare_batch_data(self, batch):
        ...
        # image = (target_imgs[0].permute((1,2,0)).cpu().clone().detach().numpy() * 255).astype(np.uint8)
        # cond_numpy_image = (cond_imgs[0].permute((1,2,0)).cpu().clone().detach().numpy() * 255).astype(np.uint8)
        # from PIL import Image
        # Image.fromarray(image).save(f"./{cond_size}_target_image.png")
        # Image.fromarray(cond_numpy_image).save(f"./{cond_size}_cond_image.png")
        # print(cond_imgs.shape, target_imgs.shape)
        # 5/0
    ...
    @torch.no_grad()
    def on_on_validation_epoch_end(self):
