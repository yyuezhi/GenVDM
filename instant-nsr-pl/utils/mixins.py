...
class SaverMixin():
    ...
    def save_tif(self,filename,vol,v_min,v_max):
        vol = -1 * self.convert_data(vol)
        v_min = self.convert_data(v_min).tolist()
        v_max = self.convert_data(v_max).tolist()
        width = vol.shape[0]
        height = vol.shape[1]
        metadata = {
            'Width': width,
        ...
        imwrite(self.get_save_path(filename), self.convert_data(vol), metadata=metadata)
