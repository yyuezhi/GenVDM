...
def load_a_prediction(root_dir, test_object, imSize, view_types, load_color=False, cam_pose_dir=None,
    ...
    RT_front = np.loadtxt(glob(os.path.join(cam_pose_dir, f'2_RT.txt'))[0])  ##Change in GT
    
    RT_front_cv = RT_opengl2opencv(RT_front)   # convert normal from opengl to opencv
    for idx, view in enumerate(view_types):


        normal_filepath = os.path.join(root_dir, test_object, f'normals_{test_object}_{view}.png')
        # Load key frame
        ...
        RT = np.loadtxt(os.path.join(cam_pose_dir, f'{view}_RT.txt'))  # world2cam matrix
    ...
    return np.stack(all_images), np.stack(all_masks), np.stack(all_normals), \
        np.stack(all_normals_world), np.stack(all_poses), np.stack(all_w2cs), np.stack(ray_origins), np.stack(directions)
