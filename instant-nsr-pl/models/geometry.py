...
class MarchingCubeHelper(nn.Module):
    ...
    def forward(self, level, threshold=0.):
        ...
        # import trimesh
        # mesh = trimesh.Trimesh(vertices=verts.detach().clone().cpu().numpy(), faces=faces.detach().clone().cpu().numpy()).export("a.obj")
        # imwrite("b.tif", level.detach().clone().cpu().numpy(), photometric='rgb')
        # exit(0)
        return {
            'v_pos': verts,
            't_pos_idx': faces,
...
class BaseImplicitGeometry(BaseModel):
    ...
    def isosurface(self):
        ...
        mesh_fine["v_min"] = vmin_
        mesh_fine["v_max"] = vmax_
        return mesh_fine 
